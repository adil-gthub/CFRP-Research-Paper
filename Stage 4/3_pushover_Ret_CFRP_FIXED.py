...
3D Cyclic Pushover Analysis — Model 3: CFRP-RETROFITTED degraded G+3 RCC frame.
  ...
  Retrofit = GROUND-FLOOR ONLY (storey between z = 1.25 m and z = 4.50 m):
    GF columns : full-height multi-ply CFRP wrap → restores f'cc ≈ 30 MPa
                 (degraded M20 + confinement, per CNR-DT 200 R1/2013).
                 Pink 350×350 → 6 plies   Gray 375×375 → 7 plies
    GF beams   : 1-ply soffit CFRP laminate (flexural) per ACI 440.2R-17.
                 Strain limited at ε_fd (debonding) via MinMax wrapper.
                 Shear U-wrap assumed adequate (not captured by fibre-flexure
                 element — distributed-plasticity model carries no shear DOF).
...
CORRECTED VERSION — all 5 M1 fixes applied + GF-only retrofit logic:
    ...
    FIX #6  Base concrete = M20 (degraded), not M30
    FIX #7  CFRP applied only to GF storey (cols iz=1→2 + beams iz=2)
    FIX #8  CNR-DT 200 ply count auto-calibrated to restore fcc ≈ 30 MPa
    FIX #9  Beam laminate stress limited at ACI 440.2R εfd via MinMax
    FIX #10 Docstring placement corrected (was after As-scale lines)
...
def cnr_dt200_confined(b, h, n_ply, tf, Ef, eps_fu,
    ...
    # CNR-DT 200 does not prescribe εcc directly; the choice is a numerical
    # one balancing two goals:
    #   • Keep εcc close to εco so the parabolic ascending branch retains
    #     initial stiffness ~ Ec_unconfined (otherwise initial-elastic
    #     capacity drops below the un-retrofitted Mander baseline, which is
    #     physically wrong — wrapping should never SOFTEN a column).
    #   • Keep εcc enough above εco that Concrete02's parabola → straight-
    #     descending transition stays smooth (very small εcc-εco gap creates
    #     a near-singular tangent at the peak → element-force-recovery
    #     failure observed during cyclic reversals).
    # Linear scaling εcc = εco·(fcc/fco) hits both — gives ~1.5×εco for
    # fcc/fco=1.5 (3e-3 vs 2e-3), preserves stiffness, parabola is smooth.
...
def define_confined_concrete(tag, fcc_p, eps_cc_p, fcu_p, eps_ccu_p):
    ...
    Uses Concrete02 (matches the M2 baseline Mander material) so the cyclic
    unloading-reloading behaviour is consistent between models — Concrete01
    has linear unload-to-zero with no stiffness recovery on reload, which
    artificially shrinks the hysteresis loops of the retrofitted model
    relative to the bare-degraded model and made M3 capacity appear LOWER
    than M2's (which is wrong physically). Concrete02 cycles correctly.

    Numerical stability of Concrete02 confined cores at small reversals is
    handled by:
       • tension cutoff via small ft (10 % of fcc^0.5 — only ascending)
       • lambda_c = 0.10 unloading-slope ratio (mild, avoids singular tangent)
...
# ---- Columns (degraded everywhere; CFRP-confined cores for GF only) ----
# Tags 101/102 = non-retrofitted columns (core = Mander confined M20)
...
# Tags 103/104 = GF retrofitted columns (core = CNR-DT 200 confined)
...
# ---- Beams ----
# Tags 201–205 = non-retrofitted beams; 222 = GF (Floor-1) beam with laminate.
build_fiber_section(201, 450, 300, 30, 600, 394)                   # Plinth
build_fiber_section(202, 375, 230, 30, 600, 248)                   # Floor 1 (default, unused)
...
build_fiber_section(222, 375, 230, 30, 600, 248, add_cfrp_beam=True)   # GF beam RETROFIT


def get_beam_sec(z_idx):
    if z_idx == 1: return 201
    if z_idx == 2: return 222    # GF beams (Floor-1 level) get CFRP soffit
...
def get_col_sec(perim, iz_start):
    """Return column section tag.
    GF storey columns (between iz=1 and iz=2) get CFRP wrap (103/104);
    every other storey uses the non-retrofitted section (101/102)."""
    GF_STOREY_IZ = 1     # column whose lower node is at iz=1 = ground-floor col
    if iz_start == GF_STOREY_IZ:
...
plt.plot(disp_array, shear_array, color='red', linewidth=1.0,
         label='Finite Element CFRP')
plt.axhline(0, color='black', linewidth=0.8); plt.axvline(0, color='black', linewidth=0.8)
plt.title('Hysteretic Load–Displacement Response Curve (CFRP retrofit)')
...
plt.plot(env_disp, env_shear, marker='o', color='blue',
         markersize=4, linestyle='-', label='CFRP Capacity Envelope')
plt.axhline(0, color='black', linewidth=0.8); plt.axvline(0, color='black', linewidth=0.8)
plt.title('Load–Displacement Envelope Curve (CFRP retrofit)')
plt.xlabel('Lateral Displacement (mm)'); plt.ylabel('Lateral Load (kN)')
plt.grid(True, linestyle='--', alpha=0.6); plt.legend(); plt.tight_layout()
...
plt.plot(stiff_disp, stiff_vals, marker='s', color='green',
         markersize=4, linestyle='-', label='CFRP Stiffness')
plt.title('Stiffness Degradation Curve (CFRP retrofit)')
...
plt.title('Capacity Curve – Load vs Top-Storey Displacement (CFRP retrofit)')
...
print(f"\n✅ CFRP retrofit analysis complete. Results saved to {out_dir}")
