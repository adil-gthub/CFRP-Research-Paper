...
def try_step():
    """One DisplacementControl step with algorithm fallback chain."""
    ok = ops.analyze(1)
    if ok == 0:
        return 0
...
for k, peak in enumerate(protocol):
    ...
    # BONUS: adapt step size with amplitude
    #   small amps → fine steps (capture early hysteresis)
    #   large amps → coarser steps (speed at large drift)
    amp_now   = abs(peak)
    step_size = max(0.5, amp_now / 40.0) * mm    # e.g. ±20 → 0.5; ±100 → 2.5

    dU    = step_size if target_disp > current_disp else -step_size
    steps = int(round(abs(target_disp - current_disp) / step_size))
    ops.integrator('DisplacementControl', roof_master_node, 1, dU)
    ...
    for _ in range(steps):
        ...
        shear_history.append(-base_shear / kN)     # reaction = −applied
    if failed:
        break
    peak_indices.append(len(disp_history) - 1)
    print(f"  Peak {peak:+4d} mm  (cycle "
          f"{k // 2 + 1}/{len(protocol)//2})")
...
# Stiffness degradation: secant K = |V_peak| / |d_peak|
...
print(f"\n✅ RCC analysis complete. Arrays + plots saved to {out_dir}")
