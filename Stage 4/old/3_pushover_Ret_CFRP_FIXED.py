...
def cnr_dt200_confined(b, h, n_ply, tf, Ef, eps_fu,
    ...
    # Ultimate strain (Eq. 4.29)
    eps_ccu = 0.0035 + 0.015 * math.sqrt(ratio)
    # Strain at peak — Mander-style scaling
    eps_cc  = eps_co * (1.0 + 5.0 * (fcc_pos/fco_pos - 1.0))
...
def define_confined_concrete(tag, fcc_p, eps_cc_p, fcu_p, eps_ccu_p):
    ...
    Uses Concrete01 (no tension envelope) for numerical stability under
    cyclic reversals. Concrete02 with the CNR-DT 200 post-peak slope
    creates a near-singular tangent at first reversal that breaks force-
    based element compatibility (force-recovery iteration fails). Concrete01
    has a cleaner cyclic envelope while preserving:
       • ascending-branch stiffness  (0 → fcc at εcc)
       • CNR-DT 200 peak strength    (fcc)
       • ductile post-peak plateau   (fcu = 0.85·fcc held flat past εccu)
    Tension is carried by the cover patches (mat 1) and steel — the
    core's tension contribution is negligible in any case."""
    fcc      = -abs(fcc_p)
    eps_cc   = -abs(eps_cc_p)
    fcu      = -abs(fcu_p)
    eps_ccu  = -abs(eps_ccu_p)
    ops.uniaxialMaterial('Concrete01', tag, fcc, eps_cc, fcu, eps_ccu)
...
plt.title('Capacity Curve – Load vs Storey Drift (CFRP retrofit)')
